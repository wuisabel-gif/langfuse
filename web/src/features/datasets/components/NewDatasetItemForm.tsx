...
const formatJsonValue = (value: Prisma.JsonValue | undefined): string => {
  ...
  try {
    // Deep-parse nested JSON strings so a JSON-string value (common for
    // OTLP-ingested metadata) renders as expanded, editable JSON like the
    // trace viewer, not an escaped blob (issue #14751). Clone objects first:
    // deepParseJson mutates in place and `value` may be shared React props.
    ...
    // If it's not valid JSON, stringify the value itself.
